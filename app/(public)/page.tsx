...
import {
  FeaturedAlbumCard,
  MAX_FEATURED_CARDS,
} from "@/components/FeaturedAlbumCard";
...
export default async function DiscoverPage() {
  ...
  const covers =
    result.state === "ready"
      ? shuffleEntries(result.collection, MAX_FEATURED_CARDS)
      : [];
  const ticker = covers.slice(0, 3);
  const collection =
    result.state === "ready" ? shuffleEntries(result.collection) : [];
      ...
      <div className="grid items-start gap-8 md:grid-cols-[minmax(0,1.55fr)_minmax(190px,340px)] md:gap-[clamp(24px,3vw,56px)]">
        <div className="min-w-0">
          ...
          <p className="mt-10 max-w-[520px] text-[19px] font-light leading-[1.6] text-[#443f4f]">
            O que acontecia na vida do artista, no mundo e nas paradas quando o
            disco saiu — e como ele soa agora. Nada de resenha: só contexto.
          </p>
          ...
          {result.state === "empty" && (
            <p className="mt-8 font-light text-[#443f4f]">
              Ainda não há álbuns no acervo. Busque um artista ou álbum para
              começar.
            </p>
          )}
        </div>
      ...
      {result.state === "ready" && (
        <>
          <div className="mt-24 flex flex-wrap items-end justify-between gap-x-4 gap-y-2">
            ...
            <span className="font-mono text-[10px] tracking-[0.26em] text-[#a8a2b0]">
              ROLE E ESCOLHA
            </span>
          </div>
          <div className="mt-9">
