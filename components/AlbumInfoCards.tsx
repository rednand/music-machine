...
function InfoCard({ label, value, span2 }: { label: string; value: string; span2?: boolean }) {
  return (
    <div
      className={`rounded-lg border border-white/90 bg-white/55 p-5 px-6 backdrop-blur-xl ${span2 ? "sm:col-span-2" : ""}`}
    >
...
export function AlbumInfoCards({
  ...
  return (
    <div className="grid gap-[14px]" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(min(100%, 300px), 1fr))" }}>
